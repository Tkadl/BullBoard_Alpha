...
def create_user_friendly_stock_selection(unique_symbols):
    ...
    with left_col:
        # Full column gradient background container
        st.markdown("""
        <div style="
            background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);
            padding: 20px;
            ...
            min-height: 500px;
        ">
            <h3 style='margin: 0 0 20px 0; color: white; text-align: center; font-weight: bold;'>
                📊 Your Portfolio
            </h3>
        """, unsafe_allow_html=True)
        
        if st.session_state.stock_basket:
            # Stocks selected header in white container
            st.markdown(f"""
            <div style="
                ...
                font-weight: bold;
            ">
            {len(st.session_state.stock_basket)} stocks selected:
            </div>
            """, unsafe_allow_html=True)
            
            # Display each stock in white containers
            ...
            # Empty state in white container
            st.markdown("""
            <div style="
                background: rgba(255, 255, 255, 0.95);
                padding: 20px;
    ...
    with right_col:
        ...
        if discovery_method == "⚡ Quick Categories":
            ...
            # Tip section in white container
            st.markdown("""
            <div style="
                background: rgba(255, 255, 255, 0.95);
                padding: 15px;
            ...
            if search_term:
                ...
                if matches:
                    ...
                    # Display matches in white containers
            ...
            <div style="
                ...
                font-weight: bold;
            ">
            Showing stocks {start_idx + 1}-{end_idx} of {total_items}:
            </div>
            """, unsafe_allow_html=True)

            # Display stocks for current page in white containers
        ...
        st.markdown("**Find companies by name or ticker:**")
            ...
            if search_term:
                ...
                if matches:
                    ...
                    # Show first 12 matches in 3 columns
                    cols = st.columns(3)
                    for i, (symbol, company_name) in enumerate(matches[:12]):
                        with cols[i % 3]:
                            if symbol not in st.session_state.stock_basket:
                                if st.button(f"+ {symbol}", key=f"add_{symbol}"):
                                    st.session_state.stock_basket.append(symbol)
                                    st.rerun()
                                st.caption(company_name[:20])
                            else:
                                st.button(f"✅ {symbol}", disabled=True, key=f"added_{symbol}")
                else:
            ...
            st.markdown("**Browse by sector or view all:**")
            
            browse_type = st.radio(
                "Browse type:",
                ["By Sector", "All Stocks"],
                key="browse_type"
            )
            
            if browse_type == "By Sector":
                sectors = {
                    'Technology': ['AAPL', 'MSFT', 'GOOGL', 'NVDA', 'META'],
                    'Healthcare': ['UNH', 'JNJ', 'PFE', 'ABBV', 'MRK'],
                    'Financial': ['JPM', 'BAC', 'WFC', 'GS', 'V'],
                    'Consumer': ['AMZN', 'HD', 'WMT', 'COST', 'NKE']
                }
                
                sector = st.selectbox("Choose sector:", list(sectors.keys()))
                sector_stocks = [s for s in sectors[sector] if s in unique_symbols]
                
                cols = st.columns(3)
                for i, symbol in enumerate(sector_stocks):
                    with cols[i % 3]:
                        if symbol not in st.session_state.stock_basket:
                            if st.button(f"+ {symbol}", key=f"sec_{symbol}"):
                                st.session_state.stock_basket.append(symbol)
                                st.rerun()
                        else:
                ...
                st.markdown(f"**All {len(unique_symbols)} stocks:**")
                
                # Paginated view
                page_size = 15
                total_pages = (len(unique_symbols) + page_size - 1) // page_size
                page = st.selectbox(f"Page:", range(1, total_pages + 1)) - 1
                ...
                for i, symbol in enumerate(page_stocks):
                    with cols[i % 3]:
                        if symbol not in st.session_state.stock_basket:
                            if st.button(f"+ {symbol}", key=f"all_{symbol}"):
                                st.session_state.stock_basket.append(symbol)
                                st.rerun()
                        else:
